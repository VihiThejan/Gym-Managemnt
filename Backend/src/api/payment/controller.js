...
const paymentHandling = async (req, res) => {
  ...
  try {
    ...
    if (isNaN(parsedMemberId) || isNaN(parsedPackageId) || isNaN(parsedAmount)) {
      console.error("Invalid input data:", { memberId, packageId, amount });
      return res.status(400).json({
        code: 400,
        message: 'Invalid member ID, package ID, or amount',
      });
    }
...
const createStripePaymentIntent = async (req, res) => {
  try {
    ...
    console.log('Creating payment intent for:', { amount, memberId, packageId });
